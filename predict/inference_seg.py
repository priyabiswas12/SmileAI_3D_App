...
class Inference:
    ...
    def preprocess_mesh(self, file_name,jaw_type):

        processed_path="./predict/uploads/{}/processed".format(jaw_type)
        mesh_path = "./predict/uploads/{}".format(jaw_type)
        #mesh = vedo.load(os.path.join(mesh_path, ))
        file= os.path.join(mesh_path,file_name)
        mesh = o3d.io.read_triangle_mesh(file)
        mesh.remove_duplicated_vertices() #remove duplicate verts
        mesh.remove_degenerate_triangles() #remove degenerate triangles
        d_mesh=mesh.simplify_quadric_decimation(10000) #downsampling
        o3d.io.write_triangle_mesh(os.path.join(processed_path,file_name),d_mesh)
        print("{} preprocessed".format(file_name))
    ...
    def predict_labels_lower(self, file_num):
        ...
        mesh_path = "./predict/uploads/lower/processed" # need to modify
        ...
        with torch.no_grad():
            ...
            #output_ply_path = os.path.join("predict/uploads", os.path.splitext(i_sample)[0] + "downsampled" + ".ply")
            #vedo.write(mesh_d, output_ply_path)
        ...
        #    # upsampling
            ...
            #remove tmp folder
            mesh_labeled = mesh_d.clone()
            mesh_labeled.celldata['Label'] = refine_labels
            output_vtp_path = os.path.join("predict/uploads/lower", os.path.splitext(i_sample)[0] + ".vtp")
            vedo.write(mesh_labeled, output_vtp_path)
    ...
    def predict_labels_upper(self, file_num):
            ...
            mesh_path = "./predict/uploads/upper/processed" # need to modify
            #sample_filenames = ['{}.stl'.format(i) for i in range(2,len(os.listdir(ip_path)) + 1)] # need to modify
            #i_sample='{}.stl'.format(file_num)
            i_sample=file_num
            print(i_sample)
            ...
            with torch.no_grad():
                ...
                mesh_labeled = mesh_d.clone()
                mesh_labeled.celldata['Label'] = refine_labels
                output_vtp_path = os.path.join("predict/uploads/upper", os.path.splitext(i_sample)[0] + ".vtp")
                vedo.write(mesh_labeled, output_vtp_path)
